      ...
      <div class="w-full max-w-7xl space-y-8 mx-auto mt-12">
        ...
        <div class="account-list mt-8">
          <table class="min-w-full bg-gray-800 text-white rounded-lg overflow-hidden">
            ...
            <tbody>
            <tr v-for="account in sortedPendingAccounts" :key="account.id" class="bg-gray-800 hover:bg-gray-700 transition-colors duration-200">
...
export default {
  ...
  computed: {
    ...
    sortedPendingAccounts() {
      return this.accounts
        .filter(account => account.accountStatus === 'PENDING')
        .slice()
        .sort((a, b) => {
          let result = 0;
  ...
  methods: {
    ...mapActions('admin', ['fetchAccounts', 'updateStatus']),
    sort(key) {
      if (this.sortKey === key) {
    ...
    async accept(id) {
      await this.updateStatus({id: id, status: 'ACTIVE'});
      await this.fetchAccounts();
    },
    async decline(id) {
      await this.updateStatus({id: id, status: 'DISABLED'});
      await this.fetchAccounts();
    }
  },
  created() {
