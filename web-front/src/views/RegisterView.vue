  ...
  <div class="custom-bg min-h-screen flex justify-center py-12 px-4 sm:px-6 lg:px-8">
    ...
    <div class="max-w-md w-full space-y-8">
      <div class="form-container shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <div>
          <h2 class="mt-6 text-center text-3xl font-extrabold text-white">{{ $t('auth.register.title') }}</h2>
        </div>
        <form class="mt-8 space-y-6" @submit.prevent="registerUser">
          <input type="hidden" name="remember" value="true">
          <div class="rounded-md shadow-sm -space-y-px">
            <div class="mb-4">
              <label for="first-name" class="sr-only">{{ $t('auth.register.first_name') }}</label>
            ...
            <div class="mb-4">
              <label for="last-name" class="sr-only">{{ $t('auth.register.last_name') }}</label>
            ...
            <div class="mb-4">
              <label for="confirm-password" class="sr-only">{{ $t('auth.register.confirm_password') }}</label>
          ...
          <div class="mb-4">
            <label for="user-type" class="sr-only">{{ $t('auth.register.user_type') }}</label>
            <select v-model="user.roleId" id="user-type" name="user-type" required
                    class="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 rounded-b-md focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm mb-4">
              <option value="User">{{ $t('auth.register.user') }}</option>
              <option value="Artist">{{ $t('auth.register.artist') }}</option>
            </select>
          </div>
          <div>
            <button type="submit"
